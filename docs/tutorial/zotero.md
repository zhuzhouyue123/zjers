...
Zotero是一款免费、开源的文献管理软件，它可以帮助你收集、组织、引用和分享研究资源。下面是一份Zotero软件的基本教程，涵盖了从安装到基本使用的步骤。
...
* **Zotero 连接器（Zotero Connector/Browser Extension）**：这是一个浏览器扩展程序（支持 Chrome, Firefox, Safari 等），用于一键保存网页和文献信息到你的 Zotero 库中。
...
1.  **安装桌面应用程序**：根据你的操作系统（Windows, macOS, Linux）下载并安装。
2.  **安装浏览器连接器**：根据你使用的浏览器，点击相应的链接进行安装。
3.  **创建 Zotero 账户（可选，但推荐）**：如果你想在不同设备间同步你的文献库，或者使用 Zotero 的在线功能，你需要创建一个免费账户。在 Zotero 桌面应用程序中，进入 **“编辑”** (Edit) 或 **“Zotero”** 菜单 $\rightarrow$ **“偏好设置”** (Preferences) $\rightarrow$ **“同步”** (Sync) 选项卡，输入你的账户信息进行同步设置。但是免费的zotero仓库同步空间有限制，如果有更多文献的同步需求，可以自行配置WebDAV网络储存。
...
* **通过浏览器连接器保存**：
    * 当你在学术数据库（如 Google Scholar, Web of Science, CNKI, 数据库官网）或在线期刊网站浏览一篇论文、书籍或网页时，观察浏览器地址栏右侧的 Zotero 连接器图标（通常是一个书本、文件夹或页面图标）。
    
    * **点击该图标**。Zotero 会自动识别页面上的元数据（标题、作者、期刊、DOI等）并将该条目保存到你的 Zotero 库中。
...
* **手动添加**：
    * 点击工具栏上的 **“添加条目”图标**（$+$），选择你要添加的文献类型（如“期刊文章”、“书籍”）。
...
* **收藏夹 (Collections)**：
    ...
    * 你可以将文献条目拖放到不同的收藏夹中。**注意：** 一篇文献可以属于多个收藏夹，它仍然只占用一个存储空间。
* **标签 (Tags)**：
    * 选中一个或多个文献条目，在右侧窗格的 **“信息”** (Info) 标签页旁找到 **“标签”** (Tags) 标签页。
    * 你可以手动添加关键词标签，方便日后通过关键词筛选文献。
* **笔记与批注**：
    * 选中一个文献条目，在右侧窗格中选择 **“笔记”** (Notes) 标签页，你可以为该文献添加独立笔记。
    * 如果你在 Zotero 内打开 PDF 并进行高亮或批注，这些内容通常可以自动提取为笔记。
...
在你安装 Zotero 桌面应用程序时，它通常会自动在你的文字处理软件（如 Microsoft Word, Google Docs, LibreOffice）中安装一个**“Zotero”选项卡/工具栏**。如果没有，请检查 Zotero 偏好设置 $\rightarrow$ **“引用”** (Cite) $\rightarrow$ **“文字处理软件”** (Word Processors) 选项卡中是否有相关选项。

![image-20251025190448432](./assets/image-20251025190448432.png)

### 6. 插入引文与生成参考文献列表

在你使用 Word 或其他兼容软件写作时：

1.  **插入引文（这里使用Harvard Style为例）**：
    
    * 在你希望插入引文的地方，点击 Word/Docs 工具栏上的 **“添加/编辑引文”** (Add/Edit Citation) 按钮。
    
      ![image-20251025190919762](./assets/image-20251025190919762.png)

    * 首次使用时，Zotero 会要求你**选择引文格式**（如 APA, MLA, GB/T 7714-2015）。
    
      ![image-20251025190952886](./assets/image-20251025190952886.png)
    
    * 会弹出一个搜索框。在搜索框中输入你要引用的文献的标题、作者或年份，找到该文献并选中。你可以一次性添加多个引文。
...
2.  **生成参考文献列表**：
    ...
    * 当你完成写作，需要在文档末尾生成参考文献列表时，点击 Word/Docs 工具栏上的 **“添加/编辑参考文献”** (Add/Edit Bibliography) 按钮。

    * Zotero 会根据你在文档中插入的所有引文，自动生成一份完整的、格式正确的参考文献列表。
...
3.  **更改引文样式**：
    * 如果你需要更换引文格式（例如从 APA 换成 GB/T），点击工具栏上的 **“文档偏好设置”** (Document Preferences)，选择新的引文样式，Zotero 会自动更新文档中所有的引文和参考文献列表。
...
通常情况下，Zotero 插件是以 **.xpi** 为后缀的文件。你需要从插件的官方网站或GitHub页面下载你想要安装的插件的 **.xpi** 文件，并记住其存放位置。
...
- 方法一：直接拖放
  ...
  找到你下载的 .xpi 文件，直接将其拖动到刚才打开的 “Zotero 附加组件”/“插件” 窗口中，Zotero 会自动识别并提示你安装。
...
- 方法二：通过菜单安装
  ...
  3. 在弹出的文件浏览器中，选择你下载的 **.xpi** 文件，然后点击 **“打开”**。

  ![image-20251025192447876](./assets/image-20251025192447876.png)

无论使用哪种方法，Zotero 都会提示你安装插件，确认后，插件即可安装成功。你可能需要重启 Zotero 以使新插件生效。
